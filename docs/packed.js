...
const index_esm2017_version = "1.0.7";
...
// Submit Rakhi data to Firebase and handle form
document.getElementById('sendButton').addEventListener('click', () => {
    ...
    if (sisterName && brotherName && termsAccepted) {
        ...
        set(newPostRef, {
            ...
            alert('Failed to send Rakhi. Please try again.');
        });
    } else {
...
// Handle page load to display correct container based on URL token
...
function showReceiverContainer(token) {
    const rakhisRef = ref(database, 'rakhis/' + token);
    onValue(rakhisRef, (snapshot) => {
        if (snapshot.exists()) {
            ...
            document.getElementById('receiver-container').style.display = 'block';
        } else {
            alert('No Rakhi found with that token.');
            document.getElementById('form-container').style.display = 'block';
        }
    }, {
        onlyOnce: true
